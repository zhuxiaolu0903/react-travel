      ...
      description:
        coupons > 0 ? (
          <div className={styles['resource-coupon-list']}>
            <a
              className={styles['resource-coupon-item']}
              onClick={() => false}
              aria-label="100元"
            >
      ...
      <div className={styles['intro-detail-content']}>
        <Row justify="space-between">
          <Col push={1}>
            ...
            <Tooltip
              placement="bottom"
              title="本起价是可选出发日期中，按双人出行共住一间房核算的最低单人价格。产品价格会根据您所选择的出发日期、出行人数、入住酒店房型航班或交通以及所选附加项目的不同而有所差别。"
            >
              <Typography.Text
                underline
        ...
        <div className={styles['product-img-container']}>
          <Carousel autoplay ref={carouselRef} afterChange={handleAfterChange}>
            {pictures.map((item) => (
              <img src={item} width={'auto'} height={320} key={item} alt=''/>
            ))}
          </Carousel>
          <div
            className={
              openSlide
                ? `${styles['slide-tab-container']} ${styles['show']}`
                : styles['slide-tab-container']
            }
          >
            <p
            ...
            <div className={styles['slide-tab-content']}>
              ...
              >
                {pictures.map((item) => (
                  <TabPane
                    tab={<img src={item} alt="" width={92} height={52} />}
                    key={item}
                  />
                ))}
              </Tabs>
